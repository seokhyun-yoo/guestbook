...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="seokhyunyoo/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/seokhyun-yoo/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token1'
                , channel: '#교육'
                , color: 'good'
